...
### 🐛 Fixed
- **A server-owned parameter name is no longer advertised on a product tool.** `compact` and `test_mode` are consumed by this server and never forwarded to the site, so a generated tool that declared either was promising input a caller could never send. They are now dropped from every generated schema, and from its `required` list, with a warning naming the ability.
- **A bad argument no longer looks like an out-of-date tool list.** `ability_invalid_input` is what a site returns for ordinary malformed arguments, but it was treated as a sign the agent's tool catalog had gone stale — so every agent typo refetched the whole catalog, re-listed every tool on the client, and told the agent to re-read a schema that was never wrong. Only a genuinely missing ability triggers a refresh now.
...
- **GravityKit product tools (`gv_*`) now carry safety annotations and honor the delete gate.** Generated tools shipped without MCP annotations, so clients had no signal that `gv_view_delete` is destructive and would run it without confirmation, while the static `gf_delete_*` tools were both flagged and gated. Ability annotations now map onto the tool definitions (`readOnlyHint`, `destructiveHint`, `idempotentHint`), and destructive product tools require `GRAVITY_FORMS_ALLOW_DELETE=true`, the same setting that protects `gf_delete_*`. Note: sites that relied on deleting Views without that setting must now set it. Each ability's server-authored `next_steps` hints are also appended to the generated tool's description, named by the tools the agent can actually call.

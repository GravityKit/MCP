...
### Changed
- **`gf_create_form` auto-assigns field ids when omitted** (GF max+1), re-basing any compound sub-input ids onto the assigned id. A caller (or a small model driving the MCP in natural language) can describe fields without hand-numbering them; explicit ids are still preserved. Removes the most common first-try failure on form creation.

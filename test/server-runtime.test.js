...
// --- stripControlParams (#7: control params must not reach WordPress) ---

test('stripControlParams: removes compact and test_mode, keeps everything else', () => {
  const cleaned = stripControlParams({ id: 9, 1: 'hello', compact: false, test_mode: true, force: true });
  assert.deepEqual(cleaned, { id: 9, 1: 'hello', force: true });
});

test('stripControlParams: tolerates null/undefined params', () => {
  assert.deepEqual(stripControlParams(undefined), {});
  assert.deepEqual(stripControlParams(null), {});
});

test('stripControlParams: does not mutate the input (wrapHandler still reads compact)', () => {

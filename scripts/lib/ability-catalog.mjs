 ...
 * endpoint. The endpoint paginates (default per_page 50), so all pages must
 * be walked or names beyond the first page are missed.
 *
 * @param {object} wpClient - WordPressClient (has baseUrl + httpClient.request)
 * @param {{prefix?: string, perPage?: number}} [opts]
 * @returns {Promise<Set<string>>}
 */
export async function collectAbilityNames(wpClient, { prefix = 'gk-gravityview/', perPage = 100 } = {}) {
  const names = new Set();
